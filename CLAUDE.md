...
### Verification pass #2 (2026-07-20) — the packaged app was completely non-functional despite every check passing

A `/verify` pass driving the actual packaged `dist/main/index.js` build via Chrome DevTools Protocol (not typecheck/lint/build, all of which stayed green throughout) found two ship-blocking bugs that would have made the app 100% non-functional for every dentist who installed it:

1. **`preload.ts` imported `CHANNELS` from `./ipc/channels`** — under `sandbox: true` (which the app correctly wants, for security), Electron's preload context cannot `require()` separate local application files, only a curated Node built-ins allowlist. This import silently failed, leaving `window.dentiScribe` `undefined` in the real renderer and the whole app stuck on the `LoginScreen`'s `"Loading…"` state forever. Fixed by inlining the `CHANNELS` constant directly into `preload.ts` (see that file's header comment). Verified via CDP: `typeof window.dentiScribe` was `"undefined"` before, `"object"` after, with all five namespaces present.
2. **Next.js's `output: 'export'` emits root-absolute asset paths** (`/_next/static/chunks/...`, `/_next/static/css/...`). Under `win.loadFile()`'s `file://` protocol these resolve to the filesystem root (`file:///_next/...`), not relative to `out/`, so every script and stylesheet 404'd — confirmed with a CDP `Network` domain capture showing `net::ERR_FILE_NOT_FOUND` on all six asset requests. React never hydrated; the static pre-rendered "Loading…" HTML was all that ever showed, even after bug #1 was fixed. Fixed by adding `src/main/staticServer.ts` (a minimal `http.createServer` bound to `127.0.0.1` on an OS-assigned port, serving `out/` directly) and changing `src/main/index.ts`'s production path from `win.loadFile(...)` to `win.loadURL('http://127.0.0.1:<port>/index.html')`. Root-absolute paths resolve correctly against an HTTP origin, unlike `file://`.

Both bugs were invisible to `npm run typecheck`, `npm run lint`, and `npm run build:main`/`build:renderer` — all passed cleanly the entire time. They were only found by actually launching the packaged app and driving it. After both fixes, the full flow was driven end-to-end via CDP and confirmed working: first-run bootstrap (create account) → DashboardShell + SetupWizard render → skip setup → paste transcript → Format Note (fails gracefully with a network error, since Ollama isn't installed on the verification machine — expected) → add/remove a style example (persists correctly) → open/close Settings (all provider dropdowns, BAA consent checkbox, cloud API key fields present) → log out → re-login with wrong password (rejected with the generic "Invalid username or password", no user-enumeration signal) → re-login with correct password (succeeds, session data correctly did not persist across the process restart, matching the ephemeral-by-design scope).

**One polish item found along the way, not fixed yet**: error messages surfaced to the user (failed `Format Note`, failed login) show the raw Electron IPC error text verbatim — `Error invoking remote method 'scribe:formatNote': TypeError: fetch failed` — rather than a friendly message. Functionally harmless (no crash, no data loss) but worth cleaning up given the target audience is explicitly non-technical dentists; a wrapper that catches `invokeAuthenticated`'s rejections in the renderer and maps them to plain-language text (e.g. "Couldn't reach the local AI — check Setup") would fix every instance at once rather than needing a fix per call site.

### A third bug, found while re-packaging after the fixes above: every installer built before this pass was actually broken

While rebuilding the installer to ship the two fixes above, driving the *actual packaged `release/win-unpacked/DentiScribe AI.exe`* (not `electron.cmd dist/main/index.js`, which is what every prior "verified working" packaging claim in this file was actually based on) surfaced a third, independent bug: the app crashed on launch with `Uncaught Exception: Error: Cannot find module '../../common/types/auth'`.

Root cause: `package.json`'s electron-builder `files` config listed `"dist/main/**/*"`, but `tsc -p src/main/tsconfig.json` compiles both `src/main/**/*.ts` *and* `src/common/**/*.ts` (since main imports from `../common/types/*`) into sibling output folders — `dist/main/` and `dist/common/`. `dist/common/**/*` was never in the packaged `files` list, so every `require('../../common/types/...')` inside the asar failed at runtime. This had been true since packaging was first set up; it was invisible until now because every prior "packaged app launches successfully" check ran the unpacked `dist/` output directly through `node_modules/.bin/electron.cmd`, which has both `dist/main` and `dist/common` on the local filesystem side by side — never the actual asar-archived build a real dentist would install.

Fixed by changing `files` to `"dist/**/*"` (see `package.json`'s `build.files`). Also hardened `src/main/index.ts`'s startup-error handling while chasing this down: `createMainWindow()` had briefly regressed to being fire-and-forgotten (`void createMainWindow()`) inside `app.whenReady().then(...)`, which meant a rejection inside it (this exact bug, or the static-server bug above) would bypass the outer `.catch()` entirely — no dialog, no log, just a silently hung process. Fixed by `await`-ing it inside an `async` `.then()` callback so `reportStartupFailure` (which now also writes the error to `%TEMP%\dentiscribe-startup-error.log` in addition to `dialog.showErrorBox`, since the native dialog can be easy to miss/lose) reliably catches it. Rebuilt, repackaged (`npm run package`, no elevation needed — winCodeSign cache still valid), and reconfirmed via CDP against the real `release/win-unpacked/DentiScribe AI.exe`: `window.dentiScribe` is defined, the bootstrap ("Create Account") screen renders correctly, no crash.

**Takeaway for future verification passes**: "launch and check" must mean the actual `release/win-unpacked/*.exe` or the installed app — running `dist/main/index.js` through the dev `electron.cmd` binary tests the compiled TypeScript, not the packaging, and asar-packaging bugs (missing `files` entries, native-module unpacking, asar-relative path resolution) are a distinct failure class that only running the real packaged artifact will catch.

### Error messages cleaned up (2026-07-20) — no more raw IPC text shown to users

The "one polish item" flagged above (raw `Error invoking remote method 'scribe:formatNote': TypeError: fetch failed` text shown verbatim to non-technical users) is fixed. Added [`src/renderer/lib/ipcError.ts`](src/renderer/lib/ipcError.ts)'s `toFriendlyErrorMessage(error, fallback)` — strips Electron's `Error invoking remote method '<channel>': ` wrapper and the redundant `<ErrorClassName>: ` prefix underneath it, then maps `fetch failed`/`ECONNREFUSED`/`ENOTFOUND` (the local-AI-not-running case) to plain language. Every backend error message that survives the strip (`Invalid username or password`, `Account temporarily locked...`, `Deepgram is the selected transcription provider but no API key is stored yet`, etc. — see `src/main/auth/errors.ts` and each provider's `throw new Error(...)`) was already written as user-facing prose, so stripping the wrapper is sufficient for those; only the generic network-failure case needed an explicit mapping.

Applied at every catch site across `ScribePanel.tsx`, `LoginScreen.tsx`, `SettingsPanel.tsx`, and `SetupWizard.tsx` (replacing the repeated inline `err instanceof Error ? err.message : '<fallback>'` pattern), and — since a full audit turned up several `window.dentiScribe.*` calls with no `.catch()` at all (style-example add/delete/list, settings load, cloud-key save/clear, `isBootstrapNeeded`, standalone `checkStatus` on mount) — added error handling to all of them too, each wired to a visible error state rather than an unhandled rejection or an eternal "Loading…" (the `isBootstrapNeeded` and settings-load cases could otherwise hang forever on failure, the same bug class as the asset-loading fix above). Verified by actually triggering both cases at the running app's UI: a wrong-password login now shows exactly `Invalid username or password` (no wrapper), and clicking Format Note with no AI installed now shows `Couldn't reach the local AI. Make sure setup is complete (see the Setup button above) and try again.` Rebuilt and repackaged; the final `release/DentiScribe AI Setup 0.1.0.exe` includes this fix.
